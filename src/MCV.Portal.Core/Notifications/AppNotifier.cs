...
namespace MCV.Portal.Notifications
{
    public class AppNotifier : PortalDomainServiceBase, IAppNotifier
    {
        ...
        public async Task NewSubjectNotifiy(User user)
        {
            await _notificationPublisher.PublishAsync(
                AppNotificationNames.NewSubject,
                new MessageNotificationData(L("NewSubjectNotitfication")),
                severity: NotificationSeverity.Success,
                userIds: new[] { user.ToUserIdentifier() }
            );
        }
    ...
    public async Task ITServiceNotification(User user,int type)
        {
            switch (type)
            {
                case 1:
                    await _notificationPublisher.PublishAsync(
                                    ...
                                    new MessageNotificationData(L("New Request Service")),
                                    severity: NotificationSeverity.Success,
                                    userIds: new[] { user.ToUserIdentifier() }
                                );
                    break;
                ...
                new MessageNotificationData(L("Approve New Request")),
                severity: NotificationSeverity.Success,
                userIds: new[] { user.ToUserIdentifier() }
            );
                    break;
                ...
                new MessageNotificationData(L("Reject New Request")),
                severity: NotificationSeverity.Success,
                userIds: new[] { user.ToUserIdentifier() }
            );
                    break;
        ...
        public async Task ITServiceApprove(User user)
        {
            await _notificationPublisher.PublishAsync(
                AppNotificationNames.ApproveNotification,
                new MessageNotificationData(L("Approve New Request")),
